  ...
  async store ({ request, response, auth }) {
    ...
    if (auth.user.user_type == "tutor") {
      try {
        ...
        await Promise.all(req.itens.map(async item => {
          const validation2 = await validateAll(item,{
            name: "string|required",
            amount: "integer|required"
          });
          if (validation2.fails()) {
            return response.status(401).send({ message: validation2.messages() });
          }
          let itemToCreate = {
            barema_id: barema.id,
            name: item.name,
  ...
  async index({response,auth}){
    if (auth.user.user_type == "tutor") {
      try{
        ...
        await Promise.all(baremas.map(async barema => {
  ...
  async destroy({ params, request, response, auth }) {
    ...
    if (auth.user.user_type == "tutor") {
      try {
        ...
        return response
          .status(200)
          .send({ message: "Barema desativado" });
      } catch (error) {
